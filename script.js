...
class App {
  ...
  _loadMap(position) {
    ...
    this.#map = L.map('map').setView(coords, this.#mapZoomLevel); //13 is the zoom level, the higher the num the closer the zoom level
  ...
  _newWorkout(e) {
    ...
    const distance = +inputDistance.value; //+ converts to a number
    const duration = +inputDuration.value;
    const { lat, lng } = this.#mapEvent.latlng;
    let workout;
    // If workout running, creating running obj
    if (type === 'running') {
